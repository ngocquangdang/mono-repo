...
export const DEFAULT_USERS: Omit<
  UserRegistration,
  | "id"
  | "createdAt"
  | "updatedAt"
...
export const storageUtils = {
  getUsers: (): UserRegistration[] => {
    ...
    const localUsers: UserRegistration[] = storedUsers
      ? JSON.parse(storedUsers)
      : [];

    // Convert default users to full UserRegistration objects
    const defaultUsersWithIds: UserRegistration[] = DEFAULT_USERS.map(
      (defaultUser, index) => ({
        ...defaultUser,
        id: `default-${index + 1}`,
        salesDate: "",
    ...
    const mergedUsers = [...localUsers, ...defaultUsersWithIds];
...
export const apiUtils = {
  getCaptcha: async (): Promise<CaptchaResponse> => {
    try {
      ...
      return {
        ...
        error: data.error,
        ...("setCookie" in data ? { setCookie: data.setCookie } : {}),
      };
    } catch (error) {
    ...
    try {
      ...
      const cookie =
        typeof window !== "undefined"
          ? localStorage.getItem("popmart_cookie") || ""
          : "";
  ...
  getSessions: async (
    salesDateId: string
  ): Promise<{ success: boolean; sessions: Session[]; error?: string }> => {
    try {
      const config = getApiConfig();
      const controller = new AbortController();
      const timeoutId = setTimeout(() => controller.abort(), config.TIMEOUT);
      ...
      const response = await fetch(
        `${config.SESSIONS_ENDPOINT}?salesDateId=${salesDateId}`,
        {
          method: "GET",
          signal: controller.signal,
          headers: { "Content-Type": "application/json" },
  ...
  generateQR: async (
    ...
    try {
      ...
      return {
        success: data.success,
        qrImageUrl: `${config.POP_MART.BASE_URL}/${data.qrImageUrl}`,
        error: data.error,
      };
    } catch (error) {
  ...
  sendEmail: async (
    ...
    try {
      ...
      const response = await fetch(
        `${config.EMAIL_ENDPOINT}?idPhien=${idPhien}&MaThamDu=${maThamDu}`,
        {
          method: "GET",
          signal: controller.signal,
          headers: { "Content-Type": "application/json" },
  ...
  ): Promise<{ success: boolean; captchaText?: string; error?: string }> => {
    try {
      const config = getApiConfig();
      const controller = new AbortController();
      const timeoutId = setTimeout(() => controller.abort(), config.TIMEOUT);
